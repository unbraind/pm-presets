...
the erasable TypeScript `scripts/prepare-merge-driver.ts`, the canonical pm-ops launcher template, copied unchanged and checked byte-for-byte by a test lifecycle. It runs `pm merge install` only
