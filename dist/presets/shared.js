...
export function normalizeTemplateName(rawName) {
    ...
    if (!TEMPLATE_NAME_PATTERN.test(name)) {
        throw new Error(`Invalid template name "${rawName}". Expected 1-64 characters matching [A-Za-z0-9][A-Za-z0-9._-]*.`);
    }
    return name;
}
...
export function applyPreset(context, input) {
    ...
    if (!fs.existsSync(pmDir) || !fs.existsSync(settingsPath)) {
        throw new Error(`pm workspace not found. Expected settings file: ${settingsPath}\n` +
    ...
    for (const [filename, template] of Object.entries(input.templates)) {
        ...
        if (filename !== `${normalizedName}${TEMPLATE_FILE_EXTENSION}`) {
            throw new Error(`Template map key "${filename}" must match document name "${normalizedName}".`);
        }
...
function parseStoredTemplateOptions(rawOptions, templateName) {
    ...
    for (const [key, value] of Object.entries(rawOptions)) {
        ...
        throw new Error(`Template "${templateName}" contains invalid value for option "${normalizedKey}".`);
    }
